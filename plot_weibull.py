...
def plot_results_with_manual_categories(samples, model, category_map):
    ...
    for category_index, category in enumerate(categories):
        ...
        if category_index == 6:
            plt.tight_layout()
            plt.savefig('results_weibull.png')
            ...
            for i, sample in enumerate(selected_samples):
                ...
                # 绘制真实值和预测值
                plt.plot(
                    time_points, release_amounts, 'o', label=f'True {i + 1}', color=f'C{i}'
                )
                plt.plot(
                    t_fine, r_fit, '-', linewidth=2, label=f'Pred {i + 1}', color=f'C{i}'
                )
            ...
            plt.grid(False)
            plt.savefig('results_special_weibull.png')
            break
       
        # 绘制每个类别的预测曲线
        plt.subplot(2, 3, category_index + 1)
        for i, sample in enumerate(selected_samples):
            # 准备单个样本的数据
            # X_sample, y_sample = prepare_data([sample])
            # 准备单个样本的数据
...
def plot_results_with_sample40(samples, model):
    os.makedirs("pic-weibull-5", exist_ok=True)
    ...
    # 准备单个样本的数据
        ...
        plt.xlabel('Time Points (h)')
        plt.ylabel('Release Percentage (%)')
        plt.title(f'Weibull Predict {sample["drug_name"]}')
        plt.tight_layout()

        save_path = os.path.join("pic-weibull-5", f'results_{i}_weibull.png')
...
def results_csv_with_all_samples(samples, model, category_map):
    ...
    save_path = os.path.join("results_csv", "weibull_model_results.csv")
    # 保存为 CSV 文件
...
def plot_results_with_allsamples(samples, model):
    ...
    for sample_id, sample in enumerate(samples):
        ...
        for model_id, model in enumerate([model1], start=1):
            ...
            # 保存图像
            output_path = os.path.join(output_dir, f"sample_{sample_id + 1}.png")
...
if __name__ == '__main__':
    ...
    model = model1
    # plot_results_with_manual_categories(samples, model, category_map)
