...
def plot_results_with_sample40(samples, model):
    os.makedirs("pic-kp-5", exist_ok=True)
    ...
    for i in selected_samples:
        ...
        plt.title(f'K-P Predict {sample["drug_name"]}')
        plt.tight_layout()

        save_path = os.path.join("pic-kp-5", f'results_{i}_kp.png')
...
def results_csv_with_all_samples(samples, model, category_map):
    ...
    save_path = os.path.join("results_csv", "kp_model_results.csv")
    # 保存为 CSV 文件
