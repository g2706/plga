...
def model1(t, k):
    # Higuchi模型
    return 100 * (k * np.sqrt(t))


def plot_results_with_sample40(samples, model):
    os.makedirs("pic-higuchi-5", exist_ok=True)
    ...
    for i in selected_samples:
        ...
        popt, _ = curve_fit(model, time_points, release_amounts, p0=[0.1], bounds=bounds, maxfev=10000)
        ...
        plt.title(f'Higuchi Predict {sample["drug_name"]}')
        plt.tight_layout()

        save_path = os.path.join("pic-higuchi-5", f'results_{i}_higuchi.png')
...
def results_csv_with_all_samples(samples, model, category_map):
    ...
    save_path = os.path.join("results_csv", "higuchi_model_results.csv")
    # 保存为 CSV 文件
    df_results.to_csv(save_path, index=False)

    print("拟合结果已保存到 higuchi_model_results.csv")
