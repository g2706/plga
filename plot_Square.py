...
# Square root of mass模型
def model1(t, k):
    # Square模型
    return 100 * (1 - np.power(1 + k * t, 2))


def plot_results_with_sample40(samples, model):
    os.makedirs("pic-square-5", exist_ok=True)
    ...
    for i in selected_samples:
        ...
        plt.title(f'SRM Predict {sample["drug_name"]}')
        plt.tight_layout()

        save_path = os.path.join("pic-square-5", f'results_{i}_square.png')
...
def results_csv_with_all_samples(samples, model, category_map):
    ...
    save_path = os.path.join("results_csv", "square_model_results.csv")
    # 保存为 CSV 文件
    df_results.to_csv(save_path, index=False)

    print("拟合结果已保存到 square_model_results.csv")
