...
# Three second-root of mass模型
def model1(t, k):
    # Three模型
    val = 1 + k * t
    val = np.maximum(val, 1e-8)  # 确保 val 不为负数
    return 100 * (1 - np.power(val, 3/2))


def plot_results_with_sample40(samples, model):
    os.makedirs("pic-three-5", exist_ok=True)
    ...
    for i in selected_samples:
        ...
        plt.title(f'TSRM Predict {sample["drug_name"]}')
        plt.tight_layout()

        save_path = os.path.join("pic-three-5", f'results_{i}_three.png')
...
def results_csv_with_all_samples(samples, model, category_map):
    ...
    save_path = os.path.join("results_csv", "three_model_results.csv")
    # 保存为 CSV 文件
    df_results.to_csv(save_path, index=False)

    print("拟合结果已保存到 three_model_results.csv")
