...
def get_parse():
    ...
    parser.add_argument('--train_path', type=str, default='./original_data.json', help='path to FSD-MIX-CLIPS_data folder)')
...
def plot_results_with_manual_categories(history_loss, history_val_loss, samples, model, category_map):
    ...
    for category_index, category in enumerate(categories):
        ...
        if category_index == 6:
            ...
            plt.xlabel('T Points (h)')
            plt.ylabel('Release Percentage (%)')
            plt.title(f'Category {category}')
            plt.grid(False)
            plt.savefig('results_special.png')
            break
