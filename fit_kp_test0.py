...
for sample_id, sample in enumerate(samples):
    ...
    for model_id, model in enumerate([model2], start=1):
        # 使用 curve_fit 函数拟合模型
        ...
        # plt.title(f"Sample {sample_id + 1}: Korsmeyer-Peppas Model Fit")
        plt.xlabel('Time Points (h)')
        plt.ylabel('Release Percentage (%)')
        plt.title('K-P Results for Sample {}'.format(sample_id + 1))
        # plt.legend()
        plt.grid(False)
        plt.tight_layout()
        # 保存图像
        output_path = os.path.join(output_dir, 'results_{}_kp.png'.format(sample_id + 1))
...
# 保存为 CSV 文件
df_results.to_csv("kp_model_results.csv", index=False)

print("拟合结果已保存到 kp_model_results.csv")
